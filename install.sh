...
mkdir -p "$DISCOVERY_DIR" "$APPTAINER_CACHEDIR" "$APPTAINER_TMPDIR" "$CONDA_PKGS_DIRS" "$PIP_CACHE_DIR"
...
echo "--- Step 3: Building Lean Components (Inside Container) ---"
apptainer exec \
  --bind .:/root/kimina-lean-server \
  --pwd /root/kimina-lean-server \
  kimina-lean-server.sif bash setup.sh
cd -
...
echo "--- Step 6: Finalizing Project Directory ---"
mkdir -p "$DISCOVERY_DIR"

cp ../kimina_setup/submit_server.sh "$USER_PROJECT_DIR/"
cp ../kimina_setup/run_kimina.sbatch "$USER_PROJECT_DIR/"
